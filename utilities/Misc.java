...
public class Misc{
    ...
    public static void writeFactorRuns(String fileName, List<HistData> runs)
    {
        try
        {
            java.io.PrintWriter output = new java.io.PrintWriter(new java.io.FileWriter(fileName));
            output.print(stringFactorRuns(runs));
            output.close();
        }
        catch(Exception e)
        {
    ...
    public static String stringFactorRuns(List<HistData> errors)
    {
        String bigPrintStr="Number of Errors = "+errors.size();
        bigPrintStr+="DeltaT\tACT\tFERT\tLIFE\tGYM\tGLAC\n";
        for (int i=0; i<errors.size(); i++)
        {
            if (errors.get(i) instanceof FactorData)
            {
                FactorData thisR = (FactorData) errors.get(i);
                bigPrintStr+=(thisR.getDELTAT()+"\t "+thisR.getACT()+"\t "+thisR.getFERT()+"\t "+thisR.getLIFE()+"\t "+thisR.getGYM()+"\t "+thisR.getGLAC()+"\t ");
                float[] co2s =thisR.getCO2();
                for (float f: co2s)
                    bigPrintStr+= f+"\t ";
                bigPrintStr+="\n";
            }
