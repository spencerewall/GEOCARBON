...
public class GEOCARB3{
    ...
    public static void doStuff(int ndat, double[] pppm, double[] dppm, int[] iage)
    {
        ...
        double sumsq0=0.0, diff=0, stdev_dof=0;
        ...
        int St=600, dlst=4, CT=6252;
        double Ws=7.4, dlct=-3.5; 
        double kwpy=0.01, kwsy=0.01, kwgy=0.018, kwcy=0.018, Fwpa1=0.25;
        ...
        double Fwsa1=0.5, Fwga1=0.5, Fwca1=2, Fmg1=1.25;
        // Fmc is C-degassing from volcanism, metamorphism and diagenesis of carbonate
        double Fmc1=6.67;
        
        double Fmp1=0.25, Fms1=0.5;
        double ZM=12.5, RBAS=.703, FBASO=.92, RRIV=.711, FRIV=3.37;
        double[] Sr=GCSV_Data.fillSr(), fSr=GCSV_Data.fillFSr();
        
        double[] fR=new double[600];
        
        //These arrays are used only in the following loop
        { double[] Bas=new double[600], DELTOT=new double[600], DELRIV=new double[600], DELBAS=new double[600], R=new double[600], SRBAS=new double[600];
        ...
        double[] fA=new double[600], fD=new double[600], temp=new double[600], DLSOC=new double[600], DLCOC=new double[600], alphac=new double[600], fL=new double[600];
        for (int i=0; i<=56; i++)
        {
            int jj=i*10;
            for (int j=0;j<=9;j++)
        ...
        for (int iiit=0; iiit<ndel; iiit++)
        {
            ...
            for (int k1=10; k1>=1; k1--)
            {
                ...
                for(int k2=10; k2>=1; k2--)
                {
                    ...
                    for (int k3=10;k3>=1;k3--)
                    {
                        ...
                        for (int k4=10; k4>=1; k4--)
                        {
                            ...
                            double[] GEOG=new double[600], fAD = new double[600];
                            for (int iberner=571; iberner>=1; iberner--)
                            {
                                ...
                                double Fbp = (1/alphas)*((DLSOC[i-1]-dlsy)*Fwsy+(DLSOC[i-1]-dlsa)*Fwsa 
                                    +(DLSOC[i-1]-dlpy)*Fwpy+(DLSOC[i-1]-dlpa)*Fwpa +(DLSOC[i-1]-dlsa)*Fms 
                                    +(DLSOC[i-1]-dlpa)*Fmp);
                                
                                    //Fbg is burial Cflux of organic sediments  
                                double Fbg=(1/alphac[i-1])*((DLCOC[i-1]-dlcy)*Fwcy+(DLCOC[i-1]-dlca)*Fwca
                                    +(DLCOC[i-1]-dlgy)*Fwgy+(DLCOC[i-1]-dlga)*Fwga +(DLCOC[i-1]-dlca)*Fmc
                                    +(DLCOC[i-1]-dlga)*Fmg);
                ...
                System.out.println(fiveFig.format(GYM)+"\t"+fiveFig.format(LIFE)+"\t"+fiveFig.format(ACT)+"\t"+fiveFig.format(FERT)+"\t"+fiveFig.format(deltaT)+"\t"+varredmin);
            } // end do
            ...
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][0][0])+"\t");}
            System.out.println();
            System.out.println("FERT");
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][1][0])+"\t");}
            System.out.println();
            System.out.println("LIFE");
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][2][0])+"\t");}
            System.out.println();
            System.out.println("GYM");
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][3][0])+"\t");}
            System.out.println("   ");
            System.out.println("parameters for runs that fit with varred 20% from min");
            System.out.println("ACT");
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][0][1])+"\t");}
            System.out.println();
            System.out.println("FERT");
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][1][1])+"\t");}
            System.out.println();
            System.out.println("LIFE");
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][2][1])+"\t");}
            System.out.println();
            System.out.println("GYM");
            for (int i=0; i<10; i++){
                System.out.print(fiveFig.format(jkk[i][3][1])+"\t");}
            // pause
            System.out.println();
            System.out.println("writing chisq to disk\t"+deltaT);
            //write(14) deltaT;
            ...
            /**all of this above was in the actual code*/
            for (int age=0; age<58; age++)
            {
                double[] flatPP = new double[10001];
                flatPP[0] = -10*age;
                for (int k1=0;k1<10;k1++) {
                    for (int k2=0;k2<10;k2++) {
                        for (int k3=0;k3<10;k3++) {
                            for (int k4=0;k4<10;k4++) 
                            {
                                flatPP[1+k4+(10*k3)+(100*k2)+(1000*k1)]
                                        = pp[k1][k2][k3][k4][age];
                            }
                        }
                    }
                }
                output.writeLine(flatPP);
            }
